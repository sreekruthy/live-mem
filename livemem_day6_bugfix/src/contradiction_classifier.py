...
facts in a knowledge base. Read the new statement and the list of existing facts below. Each \
existing fact is labeled with its current status: ACTIVE (currently valid), SUPERSEDED \
(already replaced by something else), or UNCERTAIN (a previous update that wasn't fully \
confirmed). You may still reference a SUPERSEDED fact if the new statement is clearly about \
it (e.g. re-confirming that it's no longer valid) — status labels are shown for context, not \
to hide those facts from you.
...
Does the new statement relate to any of the existing facts above? If yes, identify the SINGLE \
existing fact it relates to MOST DIRECTLY (not a fact that merely shares a topic — pick the one \
the new statement is specifically about), and classify the relationship into EXACTLY ONE of \
these four types:
...
def _format_existing_facts(prior_atoms: List[MemoryAtom]) -> str:
    """
    Shows ALL prior memories, not just active ones, each labeled with its
    current status. This matters because a later event can reference an
    already-superseded fact (e.g. "the old assignment is no longer valid"
    re-confirming something already marked superseded) — if we hide
    superseded memories here, the model has no way to correctly identify
    what a vague reference points to, and ends up guessing among whatever
    IS visible, misattributing the reference to the wrong memory.

    (Originally this only showed active memories — found via a real bug:
    see FINDINGS.md Day 5/6 section. Fixed to show all, labeled by status.)
    """
    if not prior_atoms:
        return "(none yet)"
    lines = [f"- [{m.memory_id} | status: {m.status.upper()}] {m.content}" for m in prior_atoms]
    return "\n".join(lines)
...
def classify_and_link(new_atom: MemoryAtom, prior_atoms: List[MemoryAtom]) -> dict:
    """
    Calls the LLM once to classify new_atom's relationship to ALL prior
    memories (not just active ones — see _format_existing_facts for why).
    Returns a dict with the parsed classification, defaulting to "none"
    if the call fails or returns unparseable output (logged, not
    crashed — see _parse_classification_response).
    """
    prompt = CLASSIFICATION_PROMPT_TEMPLATE.format(
        existing_facts=_format_existing_facts(prior_atoms),
        new_text=new_atom.content,
    )

    result = chat(messages=[{"role": "user", "content": prompt}], max_tokens=900)
...
def process_scenario_memories(atoms: List[MemoryAtom]) -> dict:
    ...
    for i, atom in enumerate(atoms):
        ...
        try:
            ...
            print(f"[warn] classification call failed for {atom.memory_id} after "
                  f"retries, defaulting to 'none': {e}")
            classification = {"relation": "none", "related_memory_id": None,
                               "confidence": 0.0, "input_tokens": 0, "output_tokens": 0}
