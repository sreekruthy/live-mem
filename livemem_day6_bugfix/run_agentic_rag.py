"""
Entry point: runs Agentic RAG across all scenarios/questions.
Resumable, same pattern as the other baselines.

Usage:
    python3 run_agentic_rag.py
    python3 run_agentic_rag.py --limit 40
    python3 run_agentic_rag.py --restart
"""
import argparse
import json
import os
...
from src.agentic_rag import answer_question
...
def main():
    ...
    parser.add_argument("--output", type=str, default=f"{config.OUTPUTS_DIR}/agentic_rag_predictions.jsonl")
    ...
    print(f"Running Agentic RAG on {len(work_items)} remaining questions...")
    print("(1-2 LLM calls per question depending on whether a 2nd round is triggered.)\n")
    ...
    with open(args.output, "a", encoding="utf-8") as f:
        for scenario, question in tqdm(work_items):
            try:
                ...
                round_counts[result.get("_rounds_used", "?")] += 1
    ...
    print(f"Rounds-used distribution this run: {dict(round_counts)}")
    if errors:
